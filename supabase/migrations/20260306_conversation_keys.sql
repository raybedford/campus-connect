...
--
-- Each conversation has ONE group key that encrypts all messages
-- That group key is "wrapped" (encrypted) separately for each member
...
-- (when adding someone to the conversation)
...
-- (if they need to re-wrap with a new device key)
...
-- Policy: Users can delete their own wrapped keys (when leaving conversation)
